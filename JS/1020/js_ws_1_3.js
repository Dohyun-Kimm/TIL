...
// movieInfo -results- vote_average
let rateAverage = 0 
for (let rate of movieInfo['results']){
  //console.log(rate.vote_average);
  rateAverage += rate.vote_average
}
let voteAvg = rateAverage/movieInfo['results'].length

for (let movie of movieInfo['results']){
  if (movie.vote_average < voteAvg){
